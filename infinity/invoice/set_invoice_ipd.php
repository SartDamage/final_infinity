...
/*	////////////////particulars/////amount
	...
	*/if (($due=="0" ||$due=="0.00" || $due="") && (( $paid!=""||$paid!=null||$paid!="0" || $paid!="0.00" || $paid!="00" || $advance != "0.00" || $advance !="00" || $advance != null)|| $discount!="0" || $discount!="00" ||$discount!="0.00" || $discount != "" ) ){$payment=2;}//complete payment done
	...
	$statement=$db->prepare("INSERT INTO `ipd_bill` ( `recieptID`,`Registered_ID`, `instance_id`,  `advance`, `amount`, `paid`, `discount`,`payment_type`,`cheque_no`,`electronic_id`, `WhenEntered`, `EnteredBy`) VALUES (:RecieptID,:RegID,:PatID,:advance,:subtotal,:paid,:discount,:payment_type,:cheque_no,:transcation_id,NOW(),:AdminID) ON DUPLICATE KEY UPDATE `advance`=:advance_dup, `amount`=:subtotal_dup, `paid`=:paid_dup, `discount`=:discount,payment_type=:payment_type,cheque_no=:cheque_no,electronic_id=:transcation_id,`WhenModified`=NOW(),`ModifiedBy`=:AdminID_dup");
    ...
    foreach($FORM["particulars"] as $key => $text_field){
		...
		$state=$db->prepare("INSERT INTO `ipd_bill_particulars` ( `reciept_id`,`instance_id`, `Registered_ID`,  `no_of_days`, `particulars`, `amount`, `WhenEntered`, `EnteredBy`) VALUES (:RecieptID,:PatID,:RegID,:days,:particulars,:amount,NOW(),:AdminID) ON DUPLICATE KEY UPDATE `reciept_id`=:RecieptID_dup,`Registered_ID`=:RegID_dup,`no_of_days`=:days_dup, `particulars`=:particulars_dup, `amount`=:amount_dup, `WhenModified`=NOW(),`ModifiedBy`=:AdminID_dup");
