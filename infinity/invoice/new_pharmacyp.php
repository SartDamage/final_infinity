...
<?php include $_SERVER['DOCUMENT_ROOT'].'/include/header.php';?>
	...
	<script type='text/javascript' src='/invoice/js/barcode_scanner.js'></script>
...
<body>
	...
	<div class="row">
		<div class="col-sm-6">
		<input type="text" name="scanner" id="scanner" hidden />
  ...
  $("#scanner").focus();
			...
			avgTimeByChar: 30, // it's not a barcode if a character takes longer than 100ms
			preventDefault: true,
...
//				$('#userInput').val ($('#userInput').val()  + string);
				var test_t = document.activeElement.value;
				document.activeElement.value= test_t + string;
				//alert(field1);
		 }
			...
			function populatefields(barcode_data)
			{
				var test = barcode_data[0].model_no;
				 if ($(".delete").length < 24){
				 	debugger;
	  counter++;
        var $template = $('#table_row_template'),
			...
			$clone
				...
				.find('[name="cost[]"]').attr('value', barcode_data[0].price).end()
				.find('[name="batch[]"]').attr('value', barcode_data[0].batch_no).end()
				.find('[name="mnf[]"]').attr('value', barcode_data[0].exp_date).end()
				.find('[name="particulars[]"]').attr('value', barcode_data[0].model_no).end()
                .find('[id="role-input-select"]').focus().end();

	/*	$clone
				.find('[name="role-input-select"]').attr('name', 'role-input-select-',model_no).end()
	...
	<div class="card card-outline-info mb-3" style="padding: 5px;margin-bottom: 1rem!important;">
		...
		<table id="items">
		  ...
		  <?php 
			...
			$statement=$db->prepare("SELECT ibp.* FROM `pharmacy_bill_particulars` AS ibp
		  ...
		  ?>
		      ...
		      <td >
				<input type="text" name="qty[]" onchange="update_the_price()" value="1" class="qty form-control">
			  </td>
		      <td>
				<span>₹</span>
