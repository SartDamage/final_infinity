	...
	<div class="card card-outline-info mb-3" style="padding: 5px;margin-bottom: 1rem!important;">
                ...
                <div class="hmms_hdr_rgt col-md-6">
                    <table>
						...
						<tr>
							...
							$db = getDB();
							$state=$db->prepare("select `recieptID` FROM pharmacy_bill WHERE `instance_id`=:patid limit 1");
		...
		<table id="items">
		  ...
		  <?php 
			...
			$statement=$db->prepare("SELECT ibp.particulars,ibp.amount,ibp.amount,ibp.no_of_days FROM `pharmacy_bill_particulars` AS ibp
										WHERE ibp.instance_id=:pharmacy_bill_recieptID;");
		...
		<div id="terms">
		  <h5>Terms</h5>
		  <textarea disabled><?php echo $pharma_foot ;?></textarea>
		</div>
		</div>
	</div>
