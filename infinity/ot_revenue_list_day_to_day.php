...
$userDetails=$userClass->userDetails($session_id);
		try{
			...
			for($i=30;$i>=0;$i--){
				...
				(SELECT IFNULL(((SUM(i.amount))-(SUM(i.discount))-(SUM(i.paid))-(SUM(i.advance))),0) FROM ipd_bill i LEFT JOIN patientot o ON o.patientID=i.instance_id WHERE DATE(`WhenEntered`) = CURRENT_DATE - INTERVAL $i DAY) as ot_ipd_balance,
				...
				foreach($data as $row) {
					...
					$revenue_collection->ot_paid_ipd = $ot_paid_ipd;
					$revenue_collection->ot_paid_opd = $ot_paid_opd;
					$revenue_collection->ot_amount_opd = $ot_amount_opd;
					$revenue_collection->ot_amount_ipd = $ot_amount_ipd;
					$revenue_collection->ot_balance_ipd = $ot_balance_ipd;
					$revenue_collection->ot_balance_opd = $ot_balance_opd;
