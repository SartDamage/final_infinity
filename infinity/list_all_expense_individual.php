		...
		<br>
			<div class="card card-outline-info mb-3">
			  <div class="card-block heading_bar">
				<h5>Expense section <?php // echo $userDetails->username; ?><!--(To be used in admin)--></h5>
			  </div>
			</div>
			<div class="card card-outline-info mb-3">
				<div class="card-block">
					<div class="row justify-content-md-center">
						<a  href="/add_expense.php/" class="btn btn-outline-info mar-l-10 btn_addto" id="opd">Add Expense</a>
					</div>
				</div>
			</div>
			<!--<div class="card card-outline-info mb-3">
...
{
	trbl=$('<thead class="thead-teal"><tr class="head_row"><th class="no-sort" >Sr. No.</th><th>Date</th><th>Detail</th><th>Particulars</th><th class="no-sort">Payment <br> Details</th><th>Department<br>alloted to</th><th class="no-sort" >Amount in ₹</th></tr></thead><tbody>')
	$(myTable).append(trbl);
	 ...
	 for (var i = 0; i < json.length; i++) {//slice(0,<?php //echo $no_of_entries_displayed;?>)
					...
					$.ajax({type: "GET",
							...
							data:{ID:uid,set_ind:'1'},
							async: false,
							success: function(data){
								console.log ("data from uid_get _name "+data);
								tr.append('<td>Vendor Name : '+json[i].vendor_name+' <br> contact : '+json[i].vendor_contact+' <br> authorized for :'+data+'</td>');
								}
							});
		...
		table_name.on( 'order.dt search.dt', function () {
            ...
            cell.innerHTML = i+1;
            table_name.cell(cell).invalidate('dom');
			} );
		} ).draw();
