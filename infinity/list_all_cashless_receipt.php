...
window.addEventListener('DOMContentLoaded', function() {
  console.log('window - DOMContentLoaded - capture'); // 1st
    ...
    $.ajax({
         type: "POST",
         url: "get_all_cashless_details.php",//from global_variable
...
function parseJsonToTable(json)
{
   for (var i = 0; i < json.length; i++) {//slice(0,<?php //echo $no_of_entries_displayed;?>)
      ...
      var paid="0.00";
      var total="";
      if(json[i].total_cost == null){
        total="0.00";

      }else{
        total=json[i].total_cost;
      }
          ...
          tr.append("<td>" + json[i].receiptID + " </td>");
          tr.append("<td>" + paid + " </td>");
          tr.append("<td>" + balance + " </td>");
          tr.append("<td>" + total + " </td>");
          tr.append('<td class=""><center><button type="button" onclick="clickedview(this)" data-pat_id="'+json[i].patientID+'" data-reg_id="'+json[i].RegID+'" data-receipt_id="'+json[i].receiptID+'" class="btn btn-outline-info" title="view entry" style="width:100px"><i aria-hidden="true"></i>&nbsp;View</button></center></td>');
          $('table').append(tr);
        }
        $('#myTable').DataTable({
           "scrollX": true,
...
function clickedview(e){
  $(document).ready(function() {
    ...
    var url= "cashless_receipt.php?ID="+abc+"&regid="+xyz+"&receipt_id="+pqr;
  	window.open(url,'_blank');
    //location.href="cashless_receipt.php";
});
...
<?php
$pageTitle = "Cashless Receipt list"; // Call this in your pages' files to define the page title
