...
//echo(date("Y-m-d",$t));
	...
	/* if($type_id<>""){
		...
		$statement=$db->prepare("INSERT INTO `stock_individual`
					(`category`, `type`, `brand`, `model_no`,`vendor`, `department`, `comment`, `number_stock`, `price`, `condition_stock`,`unix_timestamp`,`batch_no`,
					`EnteredBy`,`WhenEntered`)
			VALUES  (
			:category_main,:category_type,:vendor,:brand,:model,:department,:comment,:units,:price,:condition_stock,:unix_timestamp,:batch_no,
