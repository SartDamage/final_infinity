...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
    ...
    $statement=$db->prepare("INSERT INTO `p_tsh` (`RegistrationID`, `PatientId`, 
	...
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`TSH_thyroid_stimulating_hormone`=:TSH_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':TSH', $TSH, PDO::PARAM_INT);
	$statement->bindParam(':TSH_dup', $TSH, PDO::PARAM_INT);
 	// $statement->bindParam(':T4', $T4, PDO::PARAM_INT);
	// $statement->bindParam(':T4_dup', $T4, PDO::PARAM_INT);
	// $statement->bindParam(':TSH', $TSH, PDO::PARAM_INT);
	// $statement->bindParam(':TSH_dup', $TSH, PDO::PARAM_INT);
