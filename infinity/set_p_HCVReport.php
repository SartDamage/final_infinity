...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $HCVAntibody = $FORM['ctl00_reportmaster_txtHCVAntibody'];
    ...
    $statement=$db->prepare("INSERT INTO `p_hcv` (`RegistrationID`, `PatientId`, `Hcv_antibody`,`Result`, `WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:HCVAntibody,:Result,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Hcv_antibody`=:HCVAntibody_dup,`Result`=:Result_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':HCVAntibody', $HCVAntibody, PDO::PARAM_INT);
	$statement->bindParam(':HCVAntibody_dup', $HCVAntibody, PDO::PARAM_INT);
