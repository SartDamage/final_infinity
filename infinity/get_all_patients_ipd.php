...
if($pat_id=="")
{
  ...
  prm.`avatar`,prm.`Email`,prm.`Address`,prm.`IsActive` FROM patientipd AS pipd LEFT JOIN patientregistrationmaster AS prm ON prm.RegistrationID = pipd.RegID JOIN bed_number as bn on pipd.bedno=bn.ID where Date(pipd.admit_date_time) >=:from_date AND Date(pipd.admit_date_time) <=:to_date order by pipd.whenentered DESC");
}
else
{
  ...
  where pipd.`patientID`=:pat_id AND Date(pipd.admit_date_time) >=:from_date AND Date(pipd.admit_date_time) <=:to_date
