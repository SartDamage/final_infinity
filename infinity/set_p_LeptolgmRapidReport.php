...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $LeptoIgg = $FORM['ctl00_reportmaster_txtLeptoIgg'];
    ...
    $statement=$db->prepare("INSERT INTO `p_lepto_igm_rapid` (`RegistrationID`, `PatientId`, `Leptospira_IgM`,`WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:LeptoIgg,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Leptospira_IgM`=:LeptoIgg_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':LeptoIgg', $LeptoIgg, PDO::PARAM_INT);
	$statement->bindParam(':LeptoIgg_dup', $LeptoIgg, PDO::PARAM_INT);
