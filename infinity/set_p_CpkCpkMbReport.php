...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $Cpk = $FORM['ctl00_reportmaster_txtCpk'];
    $Cpk_mb = $FORM['ctl00_reportmaster_txtCpkmb'];
    ...
    $statement=$db->prepare("INSERT INTO `p_cpk_cpk_mb`(`RegistrationID`, `PatientId`, `Cpk`, `Cpk_mb`, `Sugar`, `WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:Cpk,:Cpk_mb,:Sugar,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Cpk`=:Cpk_dup,`Cpk_mb`=:Cpk_mb_dup,`Sugar`=:Sugar_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':Cpk', $Cpk, PDO::PARAM_INT);
	$statement->bindParam(':Cpk_dup', $Cpk, PDO::PARAM_INT);
 	$statement->bindParam(':Cpk_mb', $Cpk_mb, PDO::PARAM_INT);
	$statement->bindParam(':Cpk_mb_dup', $Cpk_mb, PDO::PARAM_INT);
