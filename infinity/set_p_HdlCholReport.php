...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
    ...
    $Cholesterol = $FORM['ctl00_reportmaster_txtCholesterol'];
    ...
    $statement=$db->prepare("INSERT INTO `p_hdl_chol` (`RegistrationID`, `PatientId`, `Cholesterol`,`HDL`, `WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:Cholesterol,:HDL,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Cholesterol`=:Cholesterol_dup,`HDL`=:HDL_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':HDL', $HDL, PDO::PARAM_INT);
	$statement->bindParam(':HDL_dup', $HDL, PDO::PARAM_INT);
 	$statement->bindParam(':Cholesterol', $Cholesterol, PDO::PARAM_INT);
	$statement->bindParam(':Cholesterol_dup', $Cholesterol, PDO::PARAM_INT);
