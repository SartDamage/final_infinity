...
function parseJsonToTable(json)
{
	 for (var i = 0; i < json.length; i++) {//slice(0,<?php //echo $no_of_entries_displayed;?>)
		   if(json[i].isactive=="1"){
					...
					tr = $('<tr class="tbl_row" id="'+json[i].ID+'" onclick="showDetails(this.id)" data-pat_id="'+json[i].user_id+'">');
...
/* 					tr.append("<td>" + json[i].contact + "</td>");
					...
					tr.append('<td class=""><button type="button" onclick="clickedbutton(this)" class="btn btn-outline-teal" style="width:100px"  data-uid=' + json[i].user_id + '><i class="fa fa-pencil fa-2" aria-hidden="true"></i> &nbspUpdate</button></td>');
...
function clickedbutton(button){
	...
	//alert(btn_this);
	var url= "./attendance_list.php?ID="+btn_this;
