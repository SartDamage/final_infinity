...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $DengueIgG = $FORM['ctl00_reportmaster_txtDengueIgG'];
    ...
    $statement=$db->prepare("INSERT INTO `p_dengue_igg`(`RegistrationID`, `PatientId`, `Dengue_IgG`, `WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:DengueIgG,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Dengue_IgG`=:DengueIgG_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':DengueIgG', $DengueIgG, PDO::PARAM_STR);
	$statement->bindParam(':DengueIgG_dup', $DengueIgG, PDO::PARAM_STR);
