...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $DengueIgM = $FORM['ctl00_reportmaster_txtDengueIgM'];
    ...
    $statement=$db->prepare("INSERT INTO `p_dengue_igm`(`RegistrationID`, `PatientId`, `Dengue_IgM`, `WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:DengueIgM,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Dengue_IgM`=:DengueIgM_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':DengueIgM', $DengueIgM, PDO::PARAM_STR);
	$statement->bindParam(':DengueIgM_dup', $DengueIgM, PDO::PARAM_STR);
