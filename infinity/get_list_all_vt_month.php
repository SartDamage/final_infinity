...
$form = $_POST;
if(isset($_POST['year'])){$year=$_POST['year'];}else {$year='NA';}
if(isset($_POST['month'])){$month=$_POST['month'];}else {$month='NA';}
...
/* if (isset($form['start']) && is_numeric($form['start'])  && is_int($form['start'] + 0)) {
  $start = (int) $form['start'];
      ...
      {
        $db = getDB();
        $statement=$db->prepare("SELECT * FROM add_vt_details WHERE MONTH(WhenEntered)=:month and YEAR(WhenEntered)=:year ");
