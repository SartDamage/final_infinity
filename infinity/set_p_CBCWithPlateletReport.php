...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
    ...
    $statement=$db->prepare("INSERT INTO `p_cbc_with_paltelet`(`RegistrationID`, `PatientId`, `Haemoglobin`, `Rbc_Count`, `Pcv`, `Mcv`, `Mch`, `Mchc`, `Rdw`, `Total_wbc_count`, `Neutrophils`, `Lymphocytes`, `Eosinophils`, `Monocytes`, `Basophils`, `Platelet_count`, `MPV`, `Platelets_on_smear`, `Rbc_morphology`, `Wbcs_on_ps`, `WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	:Haemoglobin,:RBCCount,:PCV,:MCV,:MCH,:MCHC,:RDW,:TotalWBCCount,:Neutrophils,:Lymphocytes,:Eosinophils,:Monocytes,:Basophils,:PlateletCount,:MPV,:PlateletsonSmear,:RBCMorphology,:WBCsonPS,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Haemoglobin`=:Haemoglobin_dup,`Rbc_Count`=:RBCCount_dup,`Pcv`=:PCV_dup,`Mcv`=:MCV_dup,`Mch`=:MCH_dup,`Mchc`=:MCHC_dup,`Rdw`=:RDW_dup,`Total_wbc_count`=:TotalWBCCount_dup,`Neutrophils`=:Neutrophils_dup,`Lymphocytes`=:Lymphocytes_dup,`Eosinophils`=:Eosinophils_dup,`Monocytes`=:Monocytes_dup,`Basophils`=:Basophils_dup,`Platelet_count`=:PlateletCount_dup,`MPV`=:MPV_dup,`Platelets_on_smear`=:PlateletsonSmear_dup,`Rbc_morphology`=:RBCMorphology_dup,`Wbcs_on_ps`=:WBCsonPS_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
