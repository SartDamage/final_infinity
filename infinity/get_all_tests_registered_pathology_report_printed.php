...
$db = getDB();
$statement=$db->prepare("SELECT pprm.ID,pprm.PatientId,pprm.RegistrationID,pprm.TestCategory,pprm.TestName,pprm.Department,pprm.ConsultedBy,pprm.EnteredBy,pprm.IsActive,pprm.`WhenEntered`,pprm.payment,pprm.`LastModified`,prm.FirstName,prm.LastName,prm.Mobile,pscm.PathologySubCategoryName,pcm.PathologyCategoryName,pscm.PathologySubCategoryID,pscm.PathologyCategoryID FROM `pathopatientregistrationmaster` AS pprm 
	...
	WHERE pprm.IsActive=1 AND pprm.Report_generated='1' AND pprm.Printed > 0 order by pprm.LastModified desc");
$statement->execute();
