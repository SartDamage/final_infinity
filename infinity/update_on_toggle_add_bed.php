...
if (isset($_GET['value_active'])):
    ...
    $statement=$db->prepare("UPDATE bed_number SET IsActive = :quickVar1a where ID=:pathodrID;"); 
	$statement->bindParam(':quickVar1a', $quickVar1a, PDO::PARAM_INT);
	$statement->bindParam(':pathodrID', $pathodrID, PDO::PARAM_INT);
	$statement->execute();
	$db=null;
	$db = getDB();
	$stmt=$db->prepare("SELECT wd.IsActive FROM bed_number AS wd WHERE wd.ID=:pathodrID1");
