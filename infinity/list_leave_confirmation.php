...
window.addEventListener('DOMContentLoaded', function() {
  console.log('window - DOMContentLoaded - capture'); // 1st
		...
		$.ajax({
			   type: "POST",
			   url: "/attmanager/leave_approval_getter.php",//from global_variable
       ...
       var iconfal="<i class='fas fa-capsules'></i>";
			 ...
			 var iconfal="<i class='fas fa-tablets'></i>";
				 ...
				 var iconfal="<i class='fas fa-user'></i>";
					...
					tr.append("<td>" + json[i].useridl + "</td>");
...
/* 					tr.append("<td>" + json[i].contact + "</td>");
					...
					tr.append('<td class=""><center><button type="button" onclick="clickedupdate(this)" class="btn btn-outline-info" title="Approve" style="width:50px"  data-uid="' + json[i].id + '"><i class="fas fa-check fa-2"></i>&nbsp;</button>&nbsp;&nbsp;&nbsp;<button type="button" onclick="clickedupdate1(this)" class="btn btn-outline-info" title="Disapprove" style="width:50px"  data-uid="' + json[i].id + '"><i class="fas fa-times fa-2"></i>&nbsp;</button><!--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" onclick="clickeddelete(this)" class="btn btn-outline-danger" title="Delete entry" style="width:100px"  data-uid="' + json[i].ID + '"><i class="fa fa-trash-o fa-2" aria-hidden="true"></i>&nbsp;delete</button>--></center></td>');
					$('table').append(tr);
					}
...
function clickedupdate(e){
	$(document).ready(function() {
		...
		$.ajax({
			 type: "GET",
			 url: "/attmanager/leave_approval_updater.php",
			 data:{"dr_ID":dr_ID,"Approv":approved},
			   success: function(data)
			   {
			         location.reload();
...
function clickedupdate1(e){
	$(document).ready(function() {
		...
		$.ajax({
			 type: "GET",
			 url: "/attmanager/leave_approval_updater.php",
			 data:{"dr_ID":dr_ID,"Approv":approval},
			   success: function(data)
			   {
			         location.reload();
			            console.log("got data : "+data);
...
<?php
$pageTitle = "Pending leave approval"; // Call this in your pages' files to define the page title
