...
$db = getDB();
$statement=$db->prepare("SELECT bn.ID,pipd.`patientID`,pipd.`RegID`,pipd.`isUHID`,pipd.`UHID`,pipd.`admit_date_time`,pipd.`discharge_date_time`,pipd.`tests_Assigned`,pipd.`doctor_assigned`,pipd.`amount_deposit`,pipd.`charges`,pipd.`symptoms`,pipd.`precription`,pipd.`diagnosis`,pipd.`isMLC`,pipd.`MLC_type`,pipd.`MLC_place`,pipd.`MLC_station`,pipd.`bedno`,pipd.`wardno`,pipd.`bed_type`,pipd.`ReferredBy`,pipd.`detailcharges`,pipd.`WhenEntered`,pipd.`EnteredBy`,pipd.`WhenModified`,pipd.`ModifiedBy`,prm.`RegistrationID`,prm.`FirstName`,prm.`LastName`,prm.`Age`,prm.`Gender`,prm.`Mobile`,prm.`avatar`,prm.`Email`,prm.`Address`,prm.`IsActive` FROM patientipd AS pipd LEFT JOIN patientregistrationmaster AS prm ON prm.RegistrationID = pipd.RegID JOIN bed_number as bn on pipd.bedno=bn.ID where Date(pipd.admit_date_time) >=:from_date AND Date(pipd.admit_date_time) <=:to_date AND NULLIF(`discharge_date_time`, ' ') IS NULL order by pipd.whenentered DESC");
...
$results=$statement->fetchAll(PDO::FETCH_ASSOC);
	...
	foreach ($results as $row){
		...
		$statement=$db->prepare("SELECT 
							avd.patientID,avd.recieptID,avd.amount_paid
        ...
        $statement_1=$db->prepare("SELECT 
							avd.instance_id,avd.recieptID,avd.amount,avd.WhenEntered
