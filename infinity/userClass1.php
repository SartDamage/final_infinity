...
/* patient details */
public function addPatient($fname,$lname,$email,$alt_contact,$gender,$marital_status,$age,$contact,$height,$weight,$address,$avatar,$bloodgroup,$ICE_name,$ICE_relation,$ICE_contact,$ICE_address,$med_history,$history,$AdminID,$GI_type,$GI_number){/*,$history){*/

				$db = getDB();
				$stmt=$db->prepare ("INSERT INTO `patientregistrationmaster`( PreFix, RegistrationID, FirstName, LastName, Email, Age, Gender, EnteredBy, WhenEntered, Mobile, IsActive, Address,avatar) VALUES ( :prefix, :RegistrationID, :fname, :lname, :email, :age, :gender, :enteredby, NOW(), :mobile, :isactive, :address,:avatar)");
...
/* staff registration  */
public function addstaff($fname,$lname,$username,$password,$email,$contact,$gender,$martial_status,$dob,$designation,$address,$bloodgroup,$avatar,$ice_name,$ice_contact,$ice_address,$roleid,$bio_id,$uid){//*,$history){*/
				$db = getDB();
				$stmt=$db->prepare ("INSERT INTO `staff_ledger`(`firstname`, `lastname`, `username`, `password`, `email`, `contact`, `gender`, `matitalstatus`, `dob`, `designation`, `address`, `bloodgroup`, `avatar`, `ice_name`, `ice_contact`, `ice_address` , `enteredby` , `whenentered` , `isactive` , `roleid`,`bio_id`,`user_id`) VALUES (:fname, :lname, :username, :password, :email, :contact, :gender, :martial_status, :dob, :designation, :address, :bloodgroup, :avatar, :ice_name, :ice_contact, :ice_address, :enteredby, NOW(), :isactive, :roleid, :bio_id, :user_id)");
				...
				$stmt->bindParam("user_id", $bio_id,PDO::PARAM_STR) ;
				/*$stmt->execute();*/
				$value =$stmt->execute();
				if($value){
					/*true*/
...
$password1="0";
				$db1 = getDB();
                $stmt1=$db1->prepare ("INSERT INTO `biometric_details`(`uid`,`user_id`, `name`, `password`, `role`, `token`,`when_created`) 
                  VALUES
                  (
                   :uid,
                   :user_id,
                 ...
                 $stmt1->bindParam(':user_id', $bio_id, PDO::PARAM_STR);
                  $stmt1->bindParam(':name', $name, PDO::PARAM_STR);
                  $stmt1->bindParam(':password', $password1, PDO::PARAM_STR);
...
public function updatePatient($RegID,$fname,$lname,$email,$alt_contact,$gender,$marital_status,$age,$contact,$height,$weight,$address,$bloodgroup,$ICE_name,$ICE_relation,$ICE_contact,$ICE_address,$med_history,$history,$AdminID,$GI_type,$GI_number){
				...
				$db = null;
				$db = getDB();
				$intopatdet=$db->prepare ("UPDATE `patientdetails` SET `bloodgroup`=:bloodgroup,`height`=:height,`weight`=:weight,`marital_status`=:marital_status,`alternate_contact`=:alternate_contact,`ice_name`=:ice_name,`ice_relation`=:ice_relation,`ice_contact`=:ice_contact,`ice_address`=:ice_address,`GI_type`=:GI_type,`GI_number`=:GI_number,`history`=:history,`med_history_detail`=:med_history_detail WHERE `RegID`=:RegID");
				$intopatdet->bindParam("RegID", $RegID,PDO::PARAM_STR);
				$intopatdet->bindParam("bloodgroup", $bloodgroup,PDO::PARAM_STR);
				$intopatdet->bindParam("height", $height,PDO::PARAM_STR);
				$intopatdet->bindParam("weight", $weight,PDO::PARAM_STR);
				$intopatdet->bindParam("marital_status", $marital_status,PDO::PARAM_STR);
				...
				$intopatdet->bindParam("GI_type", $GI_type,PDO::PARAM_STR);
				$intopatdet->bindParam("GI_number", $GI_number,PDO::PARAM_STR);
				$intopatdet->bindParam("history", $med_history,PDO::PARAM_STR);
				$intopatdet->bindParam("med_history_detail", $history,PDO::PARAM_STR);
				$value =$intopatdet->execute();
				if($value){
					/*true*///successfully registered patient
					$db = getDB();
