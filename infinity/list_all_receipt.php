...
window.addEventListener('DOMContentLoaded', function() {
  console.log('window - DOMContentLoaded - capture'); // 1st
    ...
    $.ajax({
         type: "POST",
         url: "/get_all_receipt_details.php",//from global_variable
         data: {'token':1,uid:"2",'patient_id':pat_id}, //serializes the form's elements.
         success: function(data)
         {
...
function parseJsonToTable(json)
{
  ...
  var balance = "0.00";
   ...
   for (var i = 0; i < json.length; i++) {//slice(0,<?php //echo $no_of_entries_displayed;?>)
          ...
          /*tr.append("<td>" + advance + " </td>");
          tr.append("<td>" + balance + " </td>");*/
          tr.append("<td>" + json[i].total + " </td>");
          tr.append('<td class=""><center><button type="button" onclick="clickedview(this)" class="btn btn-outline-info" data-pat_id1="'+json[i].patientID+'" data-reg_id="'+json[i].RegID+'" data-receipt_id="'+json[i].recieptID+'" title="list entry" style="width:100px"><i aria-hidden="true"></i>&nbsp;View</button></center></td>');
          $('table').append(tr);
        }
        $('#myTable').DataTable({
           "scrollX": true,
...
function clickedview(e){
  $(document).ready(function() {
    ...
    var url= "receipt.php?ID="+abc+"&Reg_id="+xyz+"&receipt_id="+pqr;
...
<?php
$pageTitle = "Receipt list"; // Call this in your pages' files to define the page title
