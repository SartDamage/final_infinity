...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
	$Troponin_T = $FORM['ctl00_reportmaster_txtTROPONIN'];
    ...
    $statement=$db->prepare("INSERT INTO p_troponin_t(`RegistrationID`, `PatientId`, 
	...
	NOW(),:AdminID,'1')
		...
		Troponin_T=:Troponin_T_dup,
			
			ModifiedBy=:AdminID_dup,LastModified=NOW()"); 
	/* $statement->bindParam(':quickVar1a', $quickVar1a, PDO::PARAM_INT); */
	$statement->bindParam(':Troponin_T', $Troponin_T, PDO::PARAM_INT);
	$statement->bindParam(':Troponin_T_dup', $Troponin_T, PDO::PARAM_INT);
