...
$userDetails=$userClass->userDetails($session_id);
  ...
  $exp_dt = $form['exp_date'];
	...
	if(isset($form['tl_doctor'])){$tl_doctor=$form['tl_doctor'];}else{$tl_doctor="";}
...
//echo 'id='.$particulars.'date='.$quantity.'time='.$cost.'comments'.$price.'pay='.$paymenttype;
    ...
    $statement=$db->prepare("INSERT INTO `cashless_receipt`
      ...
      `WhenEntered`,
       ...
       ON DUPLICATE KEY UPDATE
         ...
         `WhenModified`= NOW(),
			   `authorized_doctor`=:authorized_doctor1,
			   `remark` =:remark1,
    ...
    $statement->bindParam(':EnteredBy',$AdminID);
		...
		$statement->bindParam(':authorized_doctor1',$tl_doctor);
		$statement->bindParam(':remark',$vt_remark);
		$statement->bindParam(':remark1',$vt_remark);
