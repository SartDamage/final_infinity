...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $Reticulocyte = $FORM['ctl00_reportmaster_txtReticulocyte'];
    ...
    $statement=$db->prepare("INSERT INTO `p_retic_count` (`RegistrationID`, `PatientId`, 
		Reticulocyte_count,
	...
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	Reticulocyte_count=:Reticulocyte_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':Reticulocyte', $Reticulocyte, PDO::PARAM_INT);
	$statement->bindParam(':Reticulocyte_dup', $Reticulocyte, PDO::PARAM_INT);
 	// $statement->bindParam(':Method', $Method, PDO::PARAM_INT);
	// $statement->bindParam(':Method_dup', $Method, PDO::PARAM_INT);
	// $statement->bindParam(':DateofReading', $DateofReading, PDO::PARAM_INT);
	// $statement->bindParam(':DateofReading_dup', $DateofReading, PDO::PARAM_INT);
