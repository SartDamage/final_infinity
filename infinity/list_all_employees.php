...
/* 					tr.append("<td>" + json[i].contact + "</td>");
					...
					tr.append('<td class=""><button type="button" onclick="clickedbutton(this)" class="btn btn-outline-teal" style="width:100px"  data-uid=' + json[i].ID + '><i class="fa fa-pencil fa-2" aria-hidden="true"></i> &nbspUpdate</button>&nbsp<button type="button" onclick="clickedbutton2(this)" class="btn btn-outline-teal" style="width:150px"  data-uid=' + json[i].ID + '><i class="fa fa-user fa-2" aria-hidden="true"></i> &nbspAssign leave</button></td>');
				...
				$('#myTable').DataTable({
					 "order": [[ 3, "desc" ], [ 0, 'desc' ]],
					 "dom": "<'row'<'col-md-2'><'col-md-7'f><'col-md-1'><'col-md-2'B>>" +"<'row'<'col-md-12'tr>>" +"<'row'<'col-md-3'l><'col-md-3'i><'col-md-6'p>>",
					 "buttons": [
...
function clickedbutton2(button){
	var btn_this= button.getAttribute("data-uid");
	//alert(btn_this);
	var url= "/attmanager/singleuserholiday.php?ID="+btn_this;
	window.location.href = url;
	// Sets the new location of the current window.
	 /* $.ajax({
   type: "GET",
...
}else{$pageTitle = "List all employee's HMS";} // Call this in your pages' files to define the page title
