...
window.addEventListener('DOMContentLoaded', function() {
  console.log('window - DOMContentLoaded - capture'); // 1st
		...
		$.ajax({
			   type: "POST",
			   url: "/attmanager/all_user_present.php",//from global_variable
...
function parseJsonToTable(json)
{
	 for (var i = 0; i < json.length; i++) {//slice(0,<?php //echo $no_of_entries_displayed;?>)
		 var iconfal="<i class='fas fa-user'></i>";
					...
					tr = $('<tr class="tbl_row" id="'+json[i].user_name+'"  onclick="showDetails(this.id)"  data-pat_id="'+json[i].user_name+'">');
				if(json[i].currintime ==0){
					tr.append("<td align='center' style='background-color:#ff000069'><span style='color:white'>"+iconfal+"</td>");
				
					tr.append("<td style='background-color:#ff000069'> <span style='color:white'>" + json[i].user_name + "</td>");
					tr.append("<td style='background-color:#ff000069'> <span style='color:white'>" + json[i].user_name+ "</td>");
 					tr.append("<td style='background-color:#ff000069'> <span style='color:white'>" + json[i].currdatefr+ "</td>");
				
					tr.append("<td style='background-color:#ff000069'> <span style='color:white'>Didn't Sign in yet</td>");
					...
					tr.append("<td >" + json[i].user_name+ "</td>");
 					tr.append("<td >" + json[i].currdatefr+ "</td>");
					
					
					tr.append("<td >" + json[i].currintime + "</td>");
...
<?php
$pageTitle = "List of all staff who signed in today"; // Call this in your pages' files to define the page title
