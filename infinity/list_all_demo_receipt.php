    ...
    <br>
      ...
      <div class="card card-outline-info mb-3 margin_bot_8">
        ...
        <table class="table table-striped table-hover display nowrap" id="myTable" style="width:100%">
          ...
          <input type="hidden" id="patient_main_id"/>
...
window.addEventListener('DOMContentLoaded', function() {
  console.log('window - DOMContentLoaded - capture'); // 1st
    ...
    $.ajax({
         type: "POST",
         url: "/get_all_demo_receipt_details.php",//from global_variable
...
{ debugger;
  ...
  //debugger;
          var balance=json[i].total - json[i].amount_paid;
          document.getElementById('patient_main_id').value=json[i].patientID;
          document.getElementById('reg_id').value=json[i].RegID;
          document.getElementById('Receipt_id').value=json[i].recieptID;
          ...
          tr.append("<td>" + json[i].amount_paid + " </td>");
          tr.append("<td>" + balance + " </td>");
          tr.append("<td>" + json[i].total + " </td>");
          tr.append('<td class=""><center><button type="button" onclick="clickedview(this)" data-pat_id="'+json[i].patientID+'" data-reg_id="'+json[i].RegID+'" data-receipt_id="'+json[i].recieptID+'" class="btn btn-outline-info" title="view entry" style="width:100px"><i aria-hidden="true"></i>&nbsp;View</button></center></td>');
          $('table').append(tr);
        }
        $('#myTable').DataTable({
           "scrollX": true,
...
function clickedview(e){
  $(document).ready(function() {
    var abc=e.getAttribute('data-pat_id');
    var xyz=e.getAttribute('data-reg_id');
    var pqr=e.getAttribute('data-receipt_id');
    var url= "demo_receipt.php?ID="+abc+"&regid="+xyz+"&receipt_id="+pqr;
...
<?php
$pageTitle = "Demo Receipt list"; // Call this in your pages' files to define the page title
