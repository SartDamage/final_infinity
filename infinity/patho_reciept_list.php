...
window.addEventListener('DOMContentLoaded', function() {
  ...
  $.ajax({
			   type: "POST",
			   url: "/get_patho_reciept_list.php",//from global_variableused to be /raw_reciept_list.php
...
function parseJsonToTable(json){
	...
	for (var i = 0; i < json.length; i++) {//slice(0,<?php //echo $no_of_entries_displayed;?>)
			...
			var PContact = json[i].Mobile;
			var PGender = json[i].Gender;
			var PAge = json[i].Age;
			var PRegID = json[i].RegistrationID;
			var PathoRegID = json[i].PathoRegID;
			var Payment= json[i].Payment;
			var Paymeny_status = "";
			var Tests_list="";
			var Test_amount="";
			if(Payment==0){Paymeny_status="<span style='color:red'>Un-paid</span>";}else if(Payment==2){Paymeny_status="<span style='color:green'>Paid</span>";}else if(Payment==1){Paymeny_status="<span style='color:blue'>Partially-paid</span>";}else{Paymeny_status="Un-paid";};
			for(var j=0;j<json[i].Test.length;j++){
						var sub_test_name= json[i].Test[j].PathologySubCategoryName;
						var sub_test_id=json[i].Test[j].PathologySubCategoryID;
						var PID = json[i].Test[j].PatientId;
						var PPrinted = json[i].Test[j].Printed;
						var PReport = json[i].Test[j].Report_generated;
						var PCharges = json[i].Test[j].PathologyTestCharges;
						/* var PPayment = json[i].Test[j].payment;
						if (PPayment==0){P}*/
						if(PReport==1){PReport="green";PStatus=" View/Print";}else if(PReport==0){PReport="red";PStatus="Generate";}else{PReport="red";PStatus="Generate";}
						BAmount= BAmount+Number(PCharges);
						for_test_cell += "<div title='"+`${PStatus} ${sub_test_name} Report`+"' data-patho_scname='"+`${sub_test_name}`+"' onclick='generate_report(this)' style='padding-bottom:2px;color:"+`${PReport}`+"; cursor: pointer;' class='row intable sub_test_id' data-PID='"+`${PID}`+"' data-patho_scid='"+`${sub_test_id}`+"' data-amount_test='"+json[i].Test[j].PathologyTestCharges+"'>"+(j+1)+". "+`${sub_test_name}`+"</div>";
						Tests_list += `sub_test_name_${j}=${sub_test_name}&`;
						Test_amount += `sub_test_charges_${j}=${PCharges}&`;
						
						
			}
			console.log("Totalamount : "+BAmount);
			console.log("data to br appended on table in loop : "+for_test_cell);
			console.log("Test List : "+Tests_list);
			console.log("Test Amount : "+Test_amount);
			
			//console.log("data to br appended on table : "+for_test_cell);
			...
			tr = $('<tr class="tbl_row" id="'+PRegID+'" data-pat_id="'+PRegID+'" title="Click to view '+ PName +'\'s records">');
			...
			tr.append("<td><div class='table_div'><div class='row intable'>Gender : " + PGender + "</div><div class='row intable'> Age : " + PAge + "</div><div class='row intable'>Contact : " + PName + "</div></div></td>");
			tr.append("<td><div class='table_div'>"+for_test_cell+"</div</td>");
			//tr.append("<td>" + json[i].RegistrationID + "</td>");
			tr.append("<td> ₹ " +  `${Paymeny_status}`  + "</td>");/* `${BAmount} */
			tr.append('<td class="table_row"><center><button type="button" onclick="generate_reciept(this)" class="btn btn-outline-teal" style="width:100px" data-PathoIndividualID="'+PathoRegID+'" data-Tests_list="'+`${Tests_list}`+'" data-Test_amount="'+`${Test_amount}`+'"  data-RegistrationID=' + PRegID + ' title="Generate Invoice" ><i class=" fa-stack "><i class=" fal fa-sticky-note fa-stack-2x"></i>							<i class="fal fa-bars fa-stack-1x"></i></i> Invoice</button></center></td>');
			$('table').append(tr);
			/* $('#myTable').DataTable(); */
			}
		/* 	$('#myTable').dataTable( {
...
var PathoRegID= button.getAttribute("data-PathoIndividualID");
...
//window.location="./update_patient_form.php?ID="+ID+"";
/* for bubble propogation */
var url="/invoice/invoice_pathology_new.php?RegistrationID="+`${RegID}&${subTestAmount}${subTestName}`+"PathoRegID="+`${PathoRegID}`;
console.log(url);
window.location=url;
//openInNewTab(url);
//var win = window.open(url, '_blank');
...
<?php
$pageTitle = "Pathology invoice"; // Call this in your pages' files to define the page title
