...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $Phosphourus = $FORM['ctl00_reportmaster_txtPhosphourus'];
    ...
    $statement=$db->prepare("INSERT INTO `p_sr_phosphorous` (`RegistrationID`, `PatientId`, 
		 `S_Phosphourus`, 
	...
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`S_Phosphourus`=:Phosphourus_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':Phosphourus', $Phosphourus, PDO::PARAM_INT);
	$statement->bindParam(':Phosphourus_dup', $Phosphourus, PDO::PARAM_INT);
