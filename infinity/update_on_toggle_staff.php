...
if (isset($_GET['value_active'])):
    ...
    $statement=$db->prepare("UPDATE staff_ledger SET isactive = :quickVar1a where ID=:pathodrID;"); 
	$statement->bindParam(':quickVar1a', $quickVar1a, PDO::PARAM_INT);
	$statement->bindParam(':pathodrID', $pathodrID, PDO::PARAM_INT);
	$statement->execute();
	$db=null;
	$db = getDB();
	$stmt=$db->prepare("SELECT wd.isactive FROM staff_ledger AS wd WHERE wd.ID=:pathodrID1");
