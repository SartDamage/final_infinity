...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $Creatinine = $FORM['ctl00_reportmaster_txtCreatinine'];
    ...
    $statement=$db->prepare("INSERT INTO `p_sr_creatinine` (`RegistrationID`, `PatientId`, 
		 `Creatinine`, 
	...
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Creatinine`=:Creatinine_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':Creatinine', $Creatinine, PDO::PARAM_INT);
	$statement->bindParam(':Creatinine_dup', $Creatinine, PDO::PARAM_INT);
