...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
	$FORM = $_GET;
    $Sriron = $FORM['ctl00_reportmaster_txtSriron'];
    $Srferritin = $FORM['ctl00_reportmaster_txtSrferritin'];
    $Transferrinsaturation = $FORM['ctl00_reportmaster_txtTransferrinsaturation'];
    $Totalironbindingcapacity = $FORM['ctl00_reportmaster_txtTotalironbindingcapacity'];
    ...
    $statement=$db->prepare("INSERT INTO `p_iron_studies` (`RegistrationID`, `PatientId`, `Sr_iron`,`Sr_ferritin`,`Transferrin_saturation`,`Total_iron_binding_capacity`, `WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:Sriron,:Srferritin,:Transferrinsaturation,:Totalironbindingcapacity,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Sr_iron`=:Sriron_dup,`Sr_ferritin`=:Srferritin_dup,`Transferrin_saturation`=:Transferrinsaturation_dup,`Total_iron_binding_capacity`=:Totalironbindingcapacity_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	$statement->bindParam(':Sriron', $Sriron, PDO::PARAM_INT);
	$statement->bindParam(':Sriron_dup', $Sriron, PDO::PARAM_INT);
 	$statement->bindParam(':Srferritin', $Srferritin, PDO::PARAM_INT);
	$statement->bindParam(':Srferritin_dup', $Srferritin, PDO::PARAM_INT);
	$statement->bindParam(':Transferrinsaturation', $Transferrinsaturation, PDO::PARAM_INT);
	$statement->bindParam(':Transferrinsaturation_dup', $Transferrinsaturation, PDO::PARAM_INT);
	$statement->bindParam(':Totalironbindingcapacity', $Totalironbindingcapacity, PDO::PARAM_INT);
	$statement->bindParam(':Totalironbindingcapacity_dup', $Totalironbindingcapacity, PDO::PARAM_INT);
