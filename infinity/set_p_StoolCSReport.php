...
if (isset($_GET['ctl00_reportmaster_AdminID'])):
    ...
    $TextBox1 = $FORM['ctl00_reportmaster_TextBox1'];
    ...
    $statement=$db->prepare("INSERT INTO `p_stool_c_s` (`RegistrationID`, `PatientId`, 
		  `Report`,`Specimen`, `Method`, `M_Rsult`, `Wet_mount`, `O_Result`, `Note`, 
		
		`WhenEntered`, `EnteredBy`,`IsActive`) VALUES  (:RegID,:PatID,
	
	:Report,:Specimen,:TextBox1,:Result1,:Wetmount,:Result,:Note,
	
	NOW(),:AdminID,'1')
		ON DUPLICATE KEY UPDATE	`Report`=:Report_dup,`Specimen`=:Specimen_dup,`Method`=:TextBox1_dup,`M_Rsult`=:Result1_dup,`Wet_mount`=:Wetmount_dup,`O_Result`=:Result_dup,`Note`=:Note_dup,ModifiedBy=:AdminID_dup,LastModified=NOW()");
	...
	$statement->bindParam(':TextBox1', $TextBox1, PDO::PARAM_INT);
	$statement->bindParam(':TextBox1_dup', $TextBox1, PDO::PARAM_INT);
